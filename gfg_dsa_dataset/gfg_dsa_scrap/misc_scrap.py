...
class DsaMiscScraper(GfgDsaScrap):
    def land_gfg_dsa_misc_page(self):
        self.get(const.gfg_dsa_misc_page_url)
        link_elements = self.find_elements(By.CSS_SELECTOR, ".entry-content a")
        links = [link.get_attribute("href") for link in link_elements]
        os.mkdir("gfg_dsa/misc")
        for link in links:
            ...
            try:
                ...
                with open(f"gfg_dsa/misc/gfg_dsa_misc_{title}.txt", "w") as f:
