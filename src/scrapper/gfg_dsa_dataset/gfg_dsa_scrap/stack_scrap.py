...
class DsaStackScraper(GfgDsaScrap):
    def land_gfg_dsa_stack_page(self):
        self.get(const.gfg_dsa_stack_page_url)
        link_elements = self.find_elements(By.CSS_SELECTOR, ".entry-content a")
        links = [link.get_attribute("href") for link in link_elements]
        os.mkdir("gfg_dsa/stack")
        for link in links:
            ...
            try:
                ...
                with open(f"gfg_dsa/stack/gfg_dsa_stack_{title}.txt", "w") as f:
