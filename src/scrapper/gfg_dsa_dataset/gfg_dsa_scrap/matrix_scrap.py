...
class DsaMatrixScraper(GfgDsaScrap):
    def land_gfg_dsa_matrix_page(self):
        self.get(const.gfg_dsa_matrix_page_url)
        link_elements = self.find_elements(By.CSS_SELECTOR, ".entry-content a")
        links = [link.get_attribute("href") for link in link_elements]
        os.mkdir("fg_dsa/matrix")
        for link in links:
            ...
            try:
                ...
                with open(f"gfg_dsa/matrix/gfg_dsa_matrix_{title}.txt", "w") as f:
